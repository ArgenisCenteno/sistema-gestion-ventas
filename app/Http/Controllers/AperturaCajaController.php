...
class AperturaCajaController extends Controller
{
    public function index()
    {
        ...
        if (request()->ajax()) {
            return DataTables::of($aperturas)
                ->addColumn('caja', function ($apertura) {
                    return $apertura->caja->nombre;  // O el campo que quieras mostrar de la tabla `caja`
                })
                ->addColumn('usuario', function ($apertura) {
                    return $apertura->usuario->name;  // O el campo que quieras mostrar de la tabla `user`
                })
                ->addColumn('estatus', function ($apertura) {
                    return $apertura->estatus == 1 ? 'Abierta' : 'Cerrada';  // Condición según tu estatus
                })
                ->addColumn('created_at', function ($apertura) {
                    return $apertura->created_at->format('d/m/Y H:i:s');  // Formato de fecha
                })
                ->addColumn('updated_at', function ($apertura) {
                    return $apertura->updated_at->format('d/m/Y H:i:s');  // Formato de fecha
                })
                ->addColumn('actions', function ($apertura) {
                    return view('aperturas.actions', compact('apertura'));  // Llamamos a la vista para mostrar los botones
                })
                ->make(true);
        }
